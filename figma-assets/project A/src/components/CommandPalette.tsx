...
const COMMANDS = [
  { id: 'build', icon: <Code2 size={14} />, label: 'Open Build', sub: 'Workspace', ws: 'build' as WorkspaceId },
  { id: 'memory', icon: <Brain size={14} />, label: 'Open Memory', sub: 'Workspace', ws: 'memory' as WorkspaceId },
  { id: 'knowledge', icon: <BookOpen size={14} />, label: 'Open Knowledge', sub: 'Workspace', ws: 'knowledge' as WorkspaceId },
  { id: 'canvas', icon: <LayoutGrid size={14} />, label: 'Open Canvas', sub: 'Workspace', ws: 'canvas' as WorkspaceId },
  { id: 'plugins', icon: <Puzzle size={14} />, label: 'Open Plugins', sub: 'Workspace', ws: 'plugins' as WorkspaceId },
  { id: 'settings', icon: <Settings size={14} />, label: 'Open Settings', sub: 'Workspace', ws: 'settings' as WorkspaceId },
]
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <Search size={16} style={{ color: '#6b7099', flexShrink: 0 }} />
          <input
            ref={inputRef}
            value={query}
        ...
        <div style={{ maxHeight: 400, overflowY: 'auto' }}>
          ...
          <div style={{ padding: '8px 0' }}>
            <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', color: '#3a3f5c', padding: '6px 16px', textTransform: 'uppercase' }}>
              Workspaces
            </div>
            {filtered.map((item, i) => (
              <button
              ...
              >
                ...
                <span style={{ fontSize: 11, color: '#3a3f5c' }}>{item.sub}</span>
                {i === selected && <ArrowRight size={12} style={{ color: '#6366f1' }} />}
              </button>
            ))}
          </div>
          ...
          {query === '' && (
            <div style={{ padding: '4px 0 12px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', color: '#3a3f5c', padding: '8px 16px 4px', textTransform: 'uppercase' }}>
                Ask Zara
              </div>
              {AI_SUGGESTIONS.map((s, i) => (
                <button
                  ...
                  onClick={onClose}
                >
                  <Command size={11} style={{ color: '#6366f1' }} />
                  {s}
                </button>
              ))}
            </div>
