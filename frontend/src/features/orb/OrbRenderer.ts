...
//
// Uses the SACRED FrameState contract from core/frame/types.ts

import type { FrameState } from '../../core/frame/types';

export interface RendererHealth {
  status: 'healthy' | 'unhealthy';
  gpuContextStatus: 'ok' | 'lost' | 'unknown';
  fps: number;
...
export class OrbRenderer {
  ...
  private readonly rmsSmoothing = 0.15
  ...
  setThrottled(_throttled: boolean): void {
    // Throttling state is accepted but not currently used for adaptive rendering
  }
  ...
  private render(): void {
    ...
    if (!state) {
      ...
      const rmsLevel = state.audio.rmsLevel ?? 0
      this.smoothedRms = this.smoothedRms * (1 - this.rmsSmoothing) + rmsLevel * this.rmsSmoothing
      ...
      const audioReactive = Math.max(voiceLevel, this.smoothedRms)
      const glowAlpha = system.state === 'Error'
        ? 0.25
        : 0.15 + energy * 0.15 + audioReactive * 0.2

      this.drawGlow(cx, cy, radius, hue, saturation, lightness, energy, audioReactive, glowScale, glowAlpha)
      this.drawOrb(cx, cy, radius, hue, saturation, lightness, activity, cognitiveLoad, system.state, this.smoothedRms)
      this.drawInnerCore(cx, cy, radius, hue, presence, system.state, this.smoothedRms)
    }
  ...
  private drawIdleOrb(w: number, h: number): void {
    ...
    this.drawOrb(cx, cy, radius, 220, 70, 50, 0.2, 0.2, 'Idle', 0)
    this.drawInnerCore(cx, cy, radius, 220, 0.5, 'Idle', 0)
  }
  ...
  private drawOrb(
    ...
    this.ctx.fillStyle = gradient
    this.ctx.beginPath()
    const audioPulse = 1 + cognitiveLoad * 0.05 + smoothedRms * 0.15
    this.ctx.arc(cx, cy, radius * audioPulse, 0, Math.PI * 2)
    this.ctx.fill()

    const ringCount = state === 'Error' ? 4 : state === 'Speaking' || state === 'Executing' ? 3 : 2
    for (let i = 0; i < ringCount; i++) {
      const ringRadius = radius * (1.3 + i * 0.4 + activity * 0.2 + smoothedRms * 0.3)
      const alpha = 0.1 + activity * 0.15 + smoothedRms * 0.1
  ...
  private drawInnerCore(
    ...
    this.ctx.fillStyle = gradient
    this.ctx.beginPath()
    const corePulse = coreRadius * (1 + smoothedRms * 0.2)
