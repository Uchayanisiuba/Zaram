...
import {
  MessageSquare, Brain, Database, Settings,
  Activity, Code, Mic, Paperclip, HardDrive,
  Check, ChevronDown
} from 'lucide-react';
...
function App() {
    ...
    try {
      ...
      const analyze = () => {
        if (analyserRef.current) {
          ...
          setAudioLevel(Math.min(Math.sqrt(sum / dataArray.length) / 100, 1));
        }
        animationFrameRef.current = requestAnimationFrame(analyze);
      };
      analyze();
    } catch (e) { console.error("Audio analysis init failed:", e); }
  };

  // --- FIX 1: STATE MACHINE RESET ---
  const playNextInQueue = () => {
    if (audioQueueRef.current.length === 0) {
      isPlayingRef.current = false;
      // FIX: Explicitly reset state to idle when queue is empty and response is complete
    ...
    if (state !== 'speaking' && state !== 'thinking' && state !== 'generating') {
      setState('speaking');
    }
    ...
    if (audioRef.current) {
      ...
      audioRef.current.onerror = playNextInQueue;
  ...
  const sendMessage = async () => {
    ...
    setInput('');
    if (!hasInteracted) { setHasInteracted(true); initAudioAnalysis(); }
    stopAll();
    ...
    try {
      const response = await fetch('http://127.0.0.1:8000/chat', {
        ...
        body: JSON.stringify({ text: userText, model: 'gemma3:latest', personality: activePersonality }),
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (jsonData.type === 'token') {
                currentAssistantText += jsonData.content;
                setStreamingText(currentAssistantText);
                if (!hasReceivedFirstToken) { setIsLoading(false); hasReceivedFirstToken = true; }
              } else if (jsonData.type === 'audio') {
                if (jsonData.url) {
                  audioQueueRef.current.push(jsonData.url);
                  if (!isPlayingRef.current) playNextInQueue();
                }
              } else if (jsonData.type === 'done') {
                setIsResponseComplete(true);
                if (currentAssistantText.trim()) setMessages(prev => [...prev, { role: 'assistant', content: currentAssistantText }]);
                setStreamingText('');
                if (!isPlayingRef.current && audioQueueRef.current.length === 0) { setState('idle'); setAudioLevel(0); }
              } else if (jsonData.type === 'error') {
                setIsResponseComplete(true);
                setMessages(prev => [...prev, { role: 'assistant', content: `⚠️ Error: ${jsonData.content}` }]);
                setStreamingText('');
                setIsLoading(false);
                setState('error');
              }
            } catch (e) { console.error("Failed to parse SSE:", line); }
          }
        }
      }
    } catch (error) {
      // --- FIX 2: SAVE TEXT ON INTERRUPTION ---
      if (error.name === 'AbortError') {
        if (currentAssistantText.trim()) {
          setMessages(prev => [...prev, { role: 'assistant', content: currentAssistantText }]);
        }
        setStreamingText('');
        setIsResponseComplete(true);
        setState('idle');
        ...
        setIsLoading(false); setStreamingText(''); setIsResponseComplete(true);
        setMessages(prev => [...prev, { role: 'assistant', content: `⚠️ Error: ${error.message}` }]);
        setState('error'); setAudioLevel(0);
  ...
  const handleInputChange = (e) => {
    ...
    if ((state === 'speaking' || state === 'thinking') && e.target.value.length > 0) stopAll();
  };
    ...
    <div className="h-screen w-screen flex overflow-hidden bg-[#050810] text-slate-200 transition-colors duration-1000">
      ...
      <div className="w-64 flex flex-col bg-[#0a0f1c] border-r transition-colors duration-700" style={{ borderColor: `${colors.primary}20` }}>
        <div className="p-6 border-b border-white/5">
          ...
          <p className="text-[10px] mt-1 uppercase tracking-widest transition-colors duration-700" style={{ color: colors.secondary }}>INTELLIGENCE OS</p>
        </div>
      ...
      <div className="flex-1 flex flex-col relative overflow-hidden">
          ...
          <div className="w-full max-w-4xl h-full flex flex-col justify-center pointer-events-auto">
            <div className="flex-1 overflow-y-auto px-4 py-8 space-y-4">
              {messages.map((msg, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: "easeOut" }} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-4 rounded-2xl backdrop-blur-md border transition-all duration-700 ${msg.role === 'user' ? 'bg-white/5 border-white/10' : 'bg-black/40'}`} style={msg.role === 'assistant' ? { borderColor: `${colors.primary}40`, boxShadow: `0 0 20px ${colors.glow}` } : {}}>
                    <p className="text-sm leading-relaxed whitespace-pre-wrap">{msg.content}</p>
                  </div>
                </motion.div>
              ))}
              {streamingText && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease: "easeOut" }} className="flex justify-start">
                  <div className="max-w-[80%] p-4 rounded-2xl backdrop-blur-md border transition-all duration-700 bg-black/40" style={{ borderColor: `${colors.primary}40`, boxShadow: `0 0 20px ${colors.glow}` }}>
        ...
        {/* Input Box */}
        <motion.div className="absolute left-1/2 -translate-x-1/2 w-[600px] z-20" initial={{ top: '50vh' }} animate={{ top: hasInteracted ? 'auto' : '50vh', bottom: hasInteracted ? '2rem' : 'auto' }} transition={{ duration: 0.8, ease: "easeInOut" }}>
          <div className="glass-panel p-2 flex items-center gap-2 rounded-full border backdrop-blur-md mx-4 transition-colors duration-700" style={{ borderColor: `${colors.primary}30`, background: 'rgba(0,0,0,0.4)' }}>
            ...
            <input type="text" value={input} onChange={handleInputChange} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder="Ask Zaram anything..." className="flex-1 bg-transparent outline-none text-sm text-white px-2" />
      ...
      <div className="w-[400px] flex flex-col gap-4 p-4 border-l bg-[#0a0f1c]/50 transition-colors duration-700" style={{ borderColor: `${colors.primary}20` }}>
        <div className="glass-panel p-5 border transition-colors duration-700 relative z-40" style={{ borderColor: `${colors.primary}20` }}>
          ...
          <div className="relative">
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-all">
              <div className="flex items-center gap-3">
            ...
            {isDropdownOpen && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-[#0a0f1c] border border-white/10 rounded-lg shadow-2xl z-50 max-h-[300px] overflow-y-auto">
                {Object.entries(personalities).map(([id, p]) => (
                  <button key={id} onClick={() => { setActivePersonality(id); setIsDropdownOpen(false); }} className={`w-full flex items-center gap-3 p-3 text-left hover:bg-white/5 transition-colors ${activePersonality === id ? 'bg-white/10' : ''}`}>
