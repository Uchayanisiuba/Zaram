...
isPlayingRef.current = false;
setIsLoading(false);
// FIX: Removed setStreamingText('') from here. 
// We now handle clearing the text in the AbortError catch block so we can save it to history first.
...
// FIX: Handle Interruptions Gracefully
if (error.name === 'AbortError') {
// The user interrupted. Save whatever text was generated so far into the chat history.
if (currentAssistantText.trim()) {
setMessages(prev => [...prev, { role: 'assistant', content: currentAssistantText }]);
}
setStreamingText('');
...
// A real network/connection error occurred
setIsLoading(false);
setStreamingText('');
setIsResponseComplete(true);
