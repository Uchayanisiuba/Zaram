...
def no_live_local_server(monkeypatch):
    """**Nothing in this file may reach a real server.**

    Measured 19 September 2026: two tests below called `stream_response` on
    the dispatch path with a model resolved to `http://127.0.0.1:1234`, and
    `OpenAICompatibleEngine` is real there — so with TabbyAPI listening on
    that port, a *unit test* asked a 27B to generate "hi". It waited on the
    card for as long as the model took, while the maintainer's Unreal session
    held the VRAM, and the suite looked hung. A test that reaches a real
    server proves nothing about the code and costs whatever the server costs.

    The fake records what reached it, which is the half these tests were
    missing: they asserted the request did *not* go to Ollama, and nothing
    asserted where it *did* go.
    """
    reached: list[tuple[str | None, str]] = []

    def fake_stream(self, prompt, system_prompt="", model=None, images=None, tools=None):
        reached.append((model or self.default_model, self.base_url))
        yield "from-openai"

    monkeypatch.setattr(OpenAICompatibleEngine, "stream_response", fake_stream)
    return reached
...
class TestTheDefaultIsAChoiceToo:
    ...
    def test_the_runtime_default_is_dispatched_not_only_stored(self, no_live_local_server):
        engine, ollama = _dispatch(
            {"lm_studio:Qwen3.8-27B-exl3-2.20bpw": "http://127.0.0.1:1234"}
        )
        ...
        assert no_live_local_server == [
            ("Qwen3.8-27B-exl3-2.20bpw", "http://127.0.0.1:1234/v1")
        ]
    ...
    def test_an_explicit_model_still_outranks_the_default(self, no_live_local_server):
        engine, ollama = _dispatch(
            {"lm_studio:Qwen3.8-27B-exl3-2.20bpw": "http://127.0.0.1:1234"}
        )
        engine.default_model = "ollama:gemma4:12b"

        out = "".join(engine.stream_response("hi", "", "lm_studio:Qwen3.8-27B-exl3-2.20bpw"))

        assert ollama.calls == []
        assert out == "from-openai"
        assert [m for m, _ in no_live_local_server] == ["Qwen3.8-27B-exl3-2.20bpw"]
